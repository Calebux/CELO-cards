...
-- NOTE on `verification_proven_here`: it can read slightly under `users`.
-- A wallet verified by LINKING to another wallet carries no event of its own —
-- the WhitelistedAdded sits on its identity root — so it resolves as verified
-- through the contract (getWhitelistedRoot) but has nothing to join to here.
-- One roster wallet is in that position today. Any gap is that, not padding.

-- ─────────────────────────────────────────────────────────────────────────────
-- 9b. The same 94, one row each — paste this instead of the aggregate above
--     when someone wants to audit the list rather than trust the count.
-- ─────────────────────────────────────────────────────────────────────────────
-- SELECT
--     p.player_name,
--     p.wallet,
