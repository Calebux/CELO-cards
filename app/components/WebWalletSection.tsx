...
function Balances({ address }: { address: `0x${string}` }) {
  ...
  const { data: gdollar } = useReadContract({
  ...
  const gdollarVal = gdollar ? parseFloat(formatUnits(gdollar, 18)).toFixed(0) : "—";
  return (
    <>
      <BalanceChip label="CELO" value={celoVal} color="#FBCC5C" />
      <BalanceChip label="G$" value={gdollarVal} color="#00C58E" />
    </>
  );
}
...
export function WebWalletSection() {
