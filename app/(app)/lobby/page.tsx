...
export default function Lobby() {
  ...
  useEffect(() => {
    ...
    const submitEntry = async () => {
      // Web/mobile: the player's own wallet signs the arena entry so their
      // address is the on-chain sender. MiniPay keeps the treasury-signed
      // flow. Any failure (not connected, no CELO, rejected) falls back to
      // the treasury path so the match is never blocked.
      let playerTxHash: string | null = null;
      const arenaActive = ARENA_ADDRESS !== "0x0000000000000000000000000000000000000000";
      if (!isMp && arenaActive && isConnected && wagmiAddress) {
        try {
          playerTxHash = await writeContractAsync({
            address: ARENA_ADDRESS,
            abi: ARENA_ABI,
            functionName: "enterMatchWithCelo",
            args: [matchIdToBytes32(`${matchId}:${playerRole}`)],
      ...
      const postEntry = (txHash: string | null) =>
        fetch("/api/season-pass/enter", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ address, matchId, role: playerRole, ...(txHash ? { txHash } : {}) }),
        }).then(async (res) => {
      ...
      try {
        await postEntry(playerTxHash);
      } catch (e) {
        // Verification of a player-signed tx failed — retry via treasury.
        if (!playerTxHash) throw e;
    ...
    void submitEntry().catch((e) => {
      rankedEntryFiredRef.current = false;
      setRankedEntryError(e instanceof Error ? e.message : "Ranked entry failed");
    });
  }, [showPayModal, selfPaid, matchMode, matchId, playerRole, isMp, isConnected, wagmiAddress, writeContractAsync]);
